/*
  ...
  1. New Tables
    - `companies` - Company profiles with industry info and style profiles
      - `id` (uuid, primary key)
    ...
    - `blogs` - Blog posts with generation metadata
      - `id` (uuid, primary key)
      - `user_id` (uuid, references auth.users)
    ...
    - `blog_images` - Images generated for blogs
      - `id` (uuid, primary key)
    ...
    - `keywords` - SEO keywords with scores
      - `id` (uuid, primary key)
      - `user_id` (uuid, references auth.users)
      - `company_id` (uuid, references companies)
      - `keyword` (text)
  ...
  2. Security
    ...
    - Policies restrict all CRUD operations to authenticated owner (auth.uid() = user_id)
*/
